...
$(function() {
	...
	$("#delete").click(function() {
		$.post("/fileboard/accounts/skydrive/delete", {id: $(".account-list table input:checked").prop("id")}, function(resp) {
			if(resp) {
				...
				$(".manage-accounts .status").html("The skydrive account " + accountName + " has been deleted");
				var accountListLength = $(".account-list table tr").length;
				if(accountListLength == 1) {
					$(".account-list table").remove();
					$(".account-list").append("<p>No accounts have been configured for Skydrive</p>");
					$(".account-list").append("<span>To configure a new account, go back to the account palette and click on Add New Account for Skydrive</span>");
	...
	$("#disallow").click(function() {
		$.post("/fileboard/removeAccess/skydrive",
